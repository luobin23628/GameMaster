#import "GMApplicationTableDataSource.h"
...
@interface GMApplicationLoadingTableViewCell : UITableViewCell
@end

@implementation GMApplicationLoadingTableViewCell
...
@interface GMApplicationTableDataSourceSection : NSObject {
@private
	GMApplicationTableDataSource *_dataSource;
...
@interface GMApplicationTableDataSource ()
- (void)sectionRequestedSectionReload:(GMApplicationTableDataSourceSection *)section animated:(BOOL)animated;
@end
...
@implementation GMApplicationTableDataSourceSection

+ (void)initialize
{
	if (self == [GMApplicationTableDataSourceSection class]) {
...
- (id)initWithDescriptor:(NSDictionary *)descriptor dataSource:(GMApplicationTableDataSource *)dataSource loadsAsynchronously:(BOOL)loadsAsynchronously
{
	if ((self = [super init])) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRow:(NSInteger)row
{
	...
	if (loadingState) {
		return [tableView dequeueReusableCellWithIdentifier:@"GMApplicationLoadingTableViewCell"] ?: [[[GMApplicationLoadingTableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"GMApplicationLoadingTableViewCell"] autorelease];
	}
	...
	if (iconSize > 0) {
		...
		if ([appList hasCachedIconOfSize:iconSize forDisplayIdentifier:displayIdentifier]) {
			...
			if (iconsToLoad)
				...
				[GMApplicationTableDataSourceSection performSelectorInBackground:@selector(loadIconsFromBackground) withObject:nil];
			}
			OSSpinLockUnlock(&spinLock);
		}
	} else {
...
@implementation GMApplicationTableDataSource
...
- (void)dealloc
{
	for (GMApplicationTableDataSourceSection *section in _sectionDescriptors) {
		[section detach];
	}
...
- (void)setSectionDescriptors:(NSArray *)sectionDescriptors
{
	...
	for (NSDictionary *descriptor in sectionDescriptors) {
		NSAutoreleasePool *pool = [[NSAutoreleasePool alloc] init];
		GMApplicationTableDataSourceSection *section = [[GMApplicationTableDataSourceSection alloc] initWithDescriptor:descriptor dataSource:self loadsAsynchronously:_loadsAsynchronously];
...
{
	GMApplicationTableDataSourceSection *section = [[GMApplicationTableDataSourceSection alloc] initWithDescriptor:sectionDescriptor dataSource:self loadsAsynchronously:_loadsAsynchronously];
...
- (void)iconLoadedFromNotification:(NSNotification *)notification
{
	...
	for (NSIndexPath *indexPath in _tableView.indexPathsForVisibleRows) {
		...
		GMApplicationTableDataSourceSection *sectionObject = [_sectionDescriptors objectAtIndex:section];
...
{
	GMApplicationTableDataSourceSection *section = [_sectionDescriptors objectAtIndex:sectionIndex];
...
- (NSInteger)numberOfSectionsInTableView:(UITableView *)tableView
{
	if (!_tableView) {
		...
		NSLog(@"GMApplicationTableDataSource warning: Assumed control over %@", tableView);
	}
...
{
	GMApplicationTableDataSourceSection *section = [_sectionDescriptors objectAtIndex:indexPath.section];
