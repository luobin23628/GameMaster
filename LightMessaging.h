...
static inline kern_return_t LMConnectionSendTwoWayArchiverObject(LMConnectionRef connection, SInt32 messageId, id<NSCoding> archiverObject, LMResponseBuffer *buffer)
{
...
static inline kern_return_t LMSendArchiverObjectReply(mach_port_t replyPort, id<NSCoding> archiverObject)
{
	if (archiverObject)
...
static inline id<NSCoding> LMResponseConsumeArchiverObject(LMResponseBuffer *buffer)
{
	uint32_t length = LMMessageGetDataLength(&buffer->message);
	id result;
	if (length) {
